...
for iTime = 2:2
  for iRegion = 1:5
    % 读取INL和CTF数据
    dataINL = readtable(fullfile(dataPath, sprintf('GNSNi_%dmin_%s_R%d_inl.csv', timePoints(iTime), baseName, iRegion)));

    dataCTF = readtable(fullfile(dataPath, sprintf('GNSNi_%dmin_%s_R%d_rho_ctf.csv', timePoints(iTime), baseName, iRegion)));
  ...
  %   plotRhoDistributions(dataINL, dataCTF);    
    ...
    % 输出调整后的数据
    outputAdjustedData(dataINL, dataCTF, dataPath, timePoints(iTime), iRegion);
  end
end
...
function outputAdjustedData(dataInl, dataCtf, outputPath, timePoint, baseName)
    ...
    % 设置输出文件名，并将数据写入CSV文件
    outputFilename = sprintf('GNSNi_%dmin_%s_R%d_rho_inl.csv', timePoint, baseName2, baseName);
    writetable(dataOutput, fullfile(outputPath, outputFilename), 'Delimiter', ' ');
end

% 脚本路径：H:\Github\MyRhinoLab\scripts\p23-GNSNi-AGG-2024\20250524_Layer2a3_benchmarks\p23_s3_generate_inl_with_rho_for_moose_0524.m