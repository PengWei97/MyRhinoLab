%% 脚本说明
% H:\Github\MyRhinoLab\scripts\p23-GNSNi-AGG-2024\20250507_Layer2a3_and_Layer1a2\p23_s3_draw_hism_rho.m
...
% 处理每个指定的时间点
for iTime = 2:2
    dataCTF = readtable(fullfile(dataPath, sprintf('GNSNi_%dmin_%s_rho.csv', timePoints(iTime), baseName{1})));
    ...
    % % 输出调整后的数据
    % outputAdjustedData(dataINL, dataCTF, dataPath, timePoints(iTime), Layers{iLayer});
end
...
%% 函数：输出调整后的数据
function outputAdjustedData(dataInl, dataCtf, outputPath, timePoint, iLayer)
    % 输出调整后的数据为CSV文件
    ...
    % 设置输出文件名，并将数据写入CSV文件
    outputFilename = sprintf('GNSNi_%dmin_%s_rho_inl.csv',timePoint, iLayer); 
