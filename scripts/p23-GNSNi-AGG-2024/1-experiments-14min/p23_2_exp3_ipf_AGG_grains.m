...
for iData = 3:3
  % istage = iData + 2;
  inputFile2 = fullfile(inputDataPath, sprintf('stage%d_AGG_grains_Eulers_v2.csv', iData));
  dataTable = readtable(inputFile2);
  phi1 = [phi1; dataTable.phi1];
  Phi = [Phi; dataTable.Phi];
  phi2 = [phi2; dataTable.phi2];
end
...
odf = calcDensity(ori);
figure(1)
plotPDF(odf, Miller({1,1,1},{1,0,0},{1,1,0}, CS), 'contourf');
figure(2)
plotIPDF(odf,[xvector,yvector,zvector])
% plotPDF(odf, Miller({100}, {110}, {111}, CS));
% 脚本路径: H:\Github\MyRhinoLab\scripts\p23-GNSNi-AGG-2024\1-experiments-14min\p23_2_exp3_ipf_AGG_grains.m
