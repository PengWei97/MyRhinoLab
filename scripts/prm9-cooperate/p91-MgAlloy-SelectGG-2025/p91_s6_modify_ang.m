...
for iTime = 1:1
  % 读取INL和CTF数据
  dataINL = readtable(fullfile(inputDir, sprintf('c1_%s_inl.csv', fileNames{iTime})));

  dataCTF = readtable(fullfile(inputDir, sprintf('c1_%s_grainType_ctf.csv', fileNames{iTime})));

  if length(dataCTF.Rho) < length(dataINL.phi1)
    GrainType = ones(length(dataINL.phi1),1).*3;
    GrainType(1:length(dataCTF.Rho),1) = dataCTF.Rho;
  end
  ...
  % 输出调整后的数据
  % outputAdjustedData(dataINL, dataCTF.Rho, inputDir, fileNames{iTime});
end
